...
async def whatsapp_ask(query: Query):
    inputs = {"messages": [("system", SYSTEM_PROMPT), ("user", query)]}
    #inputs = {"messages": [("user", query.message)]}
    stream = graph.stream(inputs, stream_mode="updates")
    tool_called_name, final_response = parse_response(stream)
    if not final_response:
        final_response = "I'm sorry, I'm having trouble generating your request right now. Please try again later."
    # Step3: Send response to the frontend
